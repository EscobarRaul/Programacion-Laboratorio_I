...
def colicion(d_tablero,pos_xy):
    ...
    if(tarjeta.cantidad_tarjertas_visibles_no_descubiertas(lista_tarjetas) < 2):
        for aux_tarjeta in lista_tarjetas:
            if aux_tarjeta["rect"].collidepoint(pos_xy): #al hacer click pones las tarjetas visibles
                aux_tarjeta["visible"]=True 
                aux_tarjeta["path_imagen"]
                d_tablero["tiempo_ultimo_destape"] = pygame.time.get_ticks()
                for aux_2_tarjeta in lista_tarjetas:
                    if(aux_tarjeta["path_imagen"] == aux_2_tarjeta["path_imagen"] and aux_2_tarjeta["visible"] and aux_tarjeta != aux_2_tarjeta):
                        aux_tarjeta["descubierto"]=True
                        aux_2_tarjeta["descubierto"]=True
                break
...
def update(d_tablero):
    ...
    if(tiempo_actual - d_tablero["tiempo_ultimo_destape"] > 3000):
        lista_tarjetas = d_tablero["l_tarjetas"]
        for aux_tarjeta in lista_tarjetas:
    ...
    if(d_tablero["tiempo_ultimo_destape"] > 0):  
        if tarjeta.math(d_tablero["l_tarjetas"]):
            d_tablero["tiempo_ultimo_destape"] = 0
